"use client";
import { blogArticles } from "@/data/blogs";
import React from "react";
import { Navigation, Pagination } from "swiper/modules";
import { Swiper, SwiperSlide } from "swiper/react";
import Link from "next/link";
import Image from "next/image";
export default function Blogs() {
  const options = {
    spaceBetween: 28,
    speed: 1000,

    slidesPerView: 4,
    breakpoints: {
      0: {
        slidesPerView: 1,
      },
      700: {
        slidesPerView: 2,
      },
      1440: {
        slidesPerView: 4,
      },
    },
  };
  return (
    <section className="section-blog tf-spacing-7 pt-0">
      <div className="tf-container">
        <div className="row">
          <div className="col-12">
            <div className="heading-section">
              <h2 className="fw-7 font-cardo wow fadeInUp" data-wow-delay="0s">
                Latest From The Blog
              </h2>
              <div className="flex items-center justify-between flex-wrap gap-10">
                <div className="sub fs-15 wow fadeInUp" data-wow-delay="0.2s">
                  Explore our latest articles on education, learning, and professional development
                </div>
                <span
                  className="tf-btn-arrow wow fadeInUp"
                  data-wow-delay="0.3s"
                >
                  View All News <i className="icon-arrow-top-right" />
                </span>
              </div>
            </div>
            <Swiper
              {...options}
              modules={[Pagination, Navigation]}
              className="swiper-container tf-sw-mobile wow fadeInUp"
              data-wow-delay="0.4s"
            >
              {blogArticles.map((article, index) => (
                <SwiperSlide className="swiper-slide" key={index}>
                  <div className="blog-article-item hover-img">
                    <div className="article-thumb image-wrap">
                      {/* Blog image - Updated with Unsplash educational images */}
                      <Image
                        className="lazyload"
                        data-src={article.imgSrc}
                        alt={article.alt || `${article.title} - Educational Blog Article`}
                        src={article.imgSrc}
                        width={1880}
                        height={1100}
                      />
                    </div>
                    <div className="article-content">
                      <div className="article-label">
                        <span>
                          {article.category}
                        </span>
                      </div>
                      <h5 className="fw-5">
                        <span>
                          {article.title}
                        </span>
                      </h5>
                      <div className="meta">
                        <div className="meta-item">
                          <i className="flaticon-calendar" />
                          <p>{article.date}</p>
                        </div>
                        <div className="meta-item">
                          <i className="flaticon-message" />
                          <p>{article.comments}</p>
                        </div>
                        <span className="meta-item">
                          <i className="flaticon-user-1" />
                          <p>{article.author}</p>
                        </span>
                      </div>
                    </div>
                  </div>
                </SwiperSlide>
              ))}
            </Swiper>
          </div>
        </div>
      </div>
    </section>
  );
}